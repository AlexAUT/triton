...
struct BufferLoadToLocalOpConversion
  ...
  matchAndRewrite(triton::amdgpu::BufferLoadToLocalOp op, OpAdaptor adaptor,
    ...
    if (!isSharedSwizzled) {
      ...
      // shared addresses. This will only emit the address calculation and not
      // the actual loads
      VectorType vecTy;
      ...
      if (!targetInfo.supportsDirectToLdsLoadBitWidth(vecBits)) {
        return rewriter.notifyMatchFailure(
            op,
            "Buffer load to local does not support the required load vector "
            "bitwidth" +
      ...
      for (int i = 0; i < shmemAddrs.size(); i++) {
        ...
        bufferEmitter.emitLoadToLds(vecTy, vecBytesVal, rsrcDesc, newOffset,
                                    flattenShmemAddrs[i], pred, op.getCache());
        if (!otherElems.empty()) {
          Value storeVal =
              packElementRangeIntoVector(rewriter, this->getTypeConverter(),
                                         loc, vecTy, otherElems, srcIdx);
  ...
  matchAndRewrite(triton::gpu::AsyncCopyGlobalToLocalOp op, OpAdaptor adaptor,
    ...
    if (isSharedSwizzled) {
      ...
      if (!targetInfo.supportsDirectToLdsLoadBitWidth(vecBits)) {
        return rewriter.notifyMatchFailure(
            op,
            "Async copy does not support the required load vector bitwidth" +
                std::to_string(vecBits));
      }
