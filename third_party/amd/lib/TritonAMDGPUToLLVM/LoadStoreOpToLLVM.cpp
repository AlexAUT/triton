...
struct DirectToLdsLoadConversionBase : public LoadStoreConversionBase {
  ...
  SmallVector<Value> emitSharedBaseAddr(RewriterBase &rewriter, Operation *op,
    ...
    emitSharedAddresses(dstTy, ldsAddrs, coalescedVecTy, true);
    return ldsAddrs;
  }
  ...
  matchAndRewrite(triton::amdgpu::BufferLoadToLocalOp op, OpAdaptor adaptor,
    ...
    for (int i = 0; i < coalescedShmemAddr.size(); i++) {
      ...
      if (hasSwizzling) {
        ...
        if (tools::getBoolEnv("TRITON_HIP_ASYNC_FAST_SWIZZLE")) {
        ...
        if (tools::getBoolEnv("TRITON_HIP_ASYNC_COPY_BYPASS_PERMUTE")) {
          ...
          offsetIn = targetInfo.shuffleIdx(rewriter, loc, offsetIn, laneOffset);
        }
