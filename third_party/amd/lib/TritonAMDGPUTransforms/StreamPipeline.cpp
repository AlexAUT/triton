...
// The four stage pipeliner tries to better interleave mfams with alu ops which
// can be co-executed on GFX9. It works for loops which have 2 dots where the
// result of the first is transformed and used by the second dot.
// The dot ops will be schedule with a distance of one and the ops in between
// will be spit into 2 parts. The first part will be scheduled to the same stage
// as the fist dot so it can interleave with the second dot. Whereas the second
...
LogicalResult pipelineLoop(scf::ForOp forOp, int numStages, int globalPrefetch,
  ...
  if (succeeded(fourStagePipeliner::checkPreconditions(forOp, numStages,
                                                       loadToInfo))) {
    schedule = fourStagePipeliner::buildSchedule(
        forOp, numStages, loadToInfo, useAsyncCopy, axisInfoAnalysis);
  } else {
    schedule = streamPipeliner::buildSchedule(forOp, numStages, loadToInfo,
