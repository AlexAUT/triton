...
public:
  ...
  LogicalResult coalesceWrites(ttg::AsyncCopyGlobalToLocalOp asyncCopy) {
    ...
    if (sizePerThread == ttg::getSizePerThread(srcEncoding) &&
        threadsPerWarp == ttg::getThreadsPerWarp(srcEncoding))
      return success();
    ...
    builder.replaceOp(asyncCopy,
                      builder.create<ttg::AsyncCopyGlobalToLocalOp>(
                          asyncCopy.getLoc(), cvtSrc.getResult(),
                          asyncCopy.getResult(), mask, asyncCopy.getOther(),
